    ...
    */
    ...
    if (!export_json) {
        for (int i = 0; i < Finger_LEDs; ++i) {
            LED[LEDparts::Finger_LEDd][3 * i + 0] =
                char(Finger_LED[i] >> 16 & 0xff);
            LED[LEDparts::Finger_LEDd][3 * i + 1] =
                char(Finger_LED[i] >> 8 & 0xff);
            LED[LEDparts::Finger_LEDd][3 * i + 2] = char(Finger_LED[i] & 0xff);
        }
        for (int i = 0; i < Lower_Energy_LEDs; ++i) {
            LED[LEDparts::Lower_Energy_LEDd][3 * i + 0] =
                char(Lower_Energy_LED[i] >> 16 & 0xff);
            LED[LEDparts::Lower_Energy_LEDd][3 * i + 1] =
                char(Lower_Energy_LED[i] >> 8 & 0xff);
            LED[LEDparts::Lower_Energy_LEDd][3 * i + 2] =
                char(Lower_Energy_LED[i] & 0xff);
        }
        for (int i = 0; i < Upper_Energy_LEDs; ++i) {
            LED[LEDparts::Upper_Energy_LEDd][3 * i + 0] =
                char(Upper_Energy_LED[i] >> 16 & 0xff);
            LED[LEDparts::Upper_Energy_LEDd][3 * i + 1] =
                char(Upper_Energy_LED[i] >> 8 & 0xff);
            LED[LEDparts::Upper_Energy_LEDd][3 * i + 2] =
                char(Upper_Energy_LED[i] & 0xff);
        }
        for (int i = 0; i < Arm_Inside_LEDs; ++i) {
            LED[LEDparts::Arm_Inside_LEDd][3 * i + 0] =
                char(Arm_Inside_LED[i] >> 16 & 0xff);
            LED[LEDparts::Arm_Inside_LEDd][3 * i + 1] =
                char(Arm_Inside_LED[i] >> 8 & 0xff);
            LED[LEDparts::Arm_Inside_LEDd][3 * i + 2] =
                char(Arm_Inside_LED[i] & 0xff);
        }
...
void output() {
    ...
    for (int i = 0; i < json.size(); ++i) {
        ...
        for (int j = 0; j < Finger_LEDs; ++j) {
            ...
            colorCode =
                (colorCode << 8) | json[i][LEDparts::Finger_LEDd][3 * j + 1];
            colorCode =
                (colorCode << 8) | json[i][LEDparts::Finger_LEDd][3 * j + 2];
            outfile << "\t\t\t\t{ \"colorCode\": " << colorCode << ","
                    << "\"alpha\": 15 }";
            if (j == Finger_LEDs - 1)
                outfile << endl;
            else
    ...
    for (int i = 0; i < json.size(); ++i) {
        ...
        for (int j = 0; j < Lower_Energy_LEDs; ++j) {
            ...
            colorCode = (colorCode << 8) |
                        json[i][LEDparts::Lower_Energy_LEDd][3 * j + 1];
            colorCode = (colorCode << 8) |
                        json[i][LEDparts::Lower_Energy_LEDd][3 * j + 2];
            outfile << "\t\t\t\t{ \"colorCode\": " << colorCode << ","
                    << "\"alpha\": 15 }";
    ...
    for (int i = 0; i < json.size(); ++i) {
        ...
        for (int j = 0; j < Upper_Energy_LEDs; ++j) {
            ...
            colorCode = (colorCode << 8) |
                        json[i][LEDparts::Upper_Energy_LEDd][3 * j + 1];
            colorCode = (colorCode << 8) |
                        json[i][LEDparts::Upper_Energy_LEDd][3 * j + 2];
            outfile << "\t\t\t\t{ \"colorCode\": " << colorCode << ","
                    << "\"alpha\": 15 }";
            cout << colorCode << endl;
    ...
    for (int i = 0; i < json.size(); ++i) {
        ...
        for (int j = 0; j < Arm_Inside_LEDs; ++j) {
            ...
            colorCode = (colorCode << 8) |
                        json[i][LEDparts::Arm_Inside_LEDd][3 * j + 1];
            colorCode = (colorCode << 8) |
                        json[i][LEDparts::Arm_Inside_LEDd][3 * j + 2];
            outfile << "\t\t\t\t{ \"colorCode\": " << colorCode << ","
                    << "\"alpha\": 15 }";
...
int main(int argc, char* argv[]) {
    init();
    if (argc > 1) export_json = (strcmp(argv[1], "output") == 0);
